...
$(document).ready(function() {
    ...
    function initApp() {
        console.log('Initializing GhostCrew Terminal v4.0 with Ghosty the AI Guide');
    ...
    function initializeChatInterface() {
        ...
        if (!chatInitialized) {
            setTimeout(() => {
                addChatMessage('bot', 'Hello! I\'m Ghosty the AI Guide. I can help you with Windows commands, explain how to perform tasks, and suggest useful commands.\n\n**Try asking me:**\n• "How do I list files?"\n• "Help with network commands"\n• "Show me system information"\n• "How do I create a folder?"\n\nWhat would you like to know?');
            ...
            <div class="terminal-session active" id="${hostId}-terminal">
                <div class="terminal-output">
                    <div class="welcome-message">
                        ...
                        <p>Ready to receive commands. Type commands below or ask Ghosty the AI Guide for help.</p>
                    </div>
                </div>
            </div>`;
    ...
    // Enhanced sendCommand function to mark messages as having executed commands
    function sendCommand() {
        const command = $('#terminal-input').val().trim();
        const messageId = $('#terminal-input').attr('data-message-id');
        
        if (command === '' || !activeHostId || !currentRemoteSessionId) {
            return;
        }
        
        // Store the command being sent with message tracking
        lastCommandSent = {
            command: command,
            timestamp: Date.now(),
        ...
        // Clear the input field and remove message tracking
        $('#terminal-input').val('').removeAttr('data-message-id').focus();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    // Mark message as having executed command if it came from a bot suggestion
                    if (messageId) {
                        markMessageAsExecuted(messageId);
                    }
    ...
    // New function to mark message as having executed command
    function markMessageAsExecuted(messageId) {
        $.ajax({
            url: 'api.php',
            type: 'POST',
            data: {
            ...
            success: function(response) {
                if (response.status === 'success') {
                    console.log('Message marked as having executed command:', messageId);
                    
                    // Update UI to show command was executed from this message
                    $(`.loaded-btn[data-suggestion-id="${messageId}"]`)
                        .removeClass('loaded-btn')
                        .addClass('executed-btn')
                        .html('<i class="fas fa-play-circle"></i> Executed')
                        .prop('title', 'A command from this message has been executed');
                    
                    // Mark all other suggestion buttons from this message as executed too
                    $(`.load-suggestion-btn[data-suggestion-id="${messageId}"]`)
                        .removeClass('load-suggestion-btn')
                        .addClass('message-executed')
                        .html('<i class="fas fa-info-circle"></i> Related')
                        .prop('title', 'Another command from this message was executed')
                        .prop('disabled', false); // Keep them clickable
                }
            },
            error: function() {
                console.log('Failed to mark message as executed:', messageId);
            }
        });
    }
    ...
    // Enhanced loadSuggestedCommand function to track message ID
    function loadSuggestedCommand(suggestionId, command) {
        ...
        // Store the message ID (suggestionId is actually the bot message ID) for execution tracking
        $('#terminal-input').attr('data-message-id', suggestionId);
    ...
    function markSuggestionAsUsed(suggestionId) {
        // This function can remain empty or be removed since we're not tracking individual suggestions anymore
        console.log('markSuggestionAsUsed called (deprecated):', suggestionId);
    }
    ...
    console.log('   ✅ Ghosty the AI Guide with contextual responses');
