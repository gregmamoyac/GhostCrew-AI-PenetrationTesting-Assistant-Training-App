...
async function loadSettings() {
    ...
    try {
        ...
        // Load AI configuration if user is admin
...
function showSettingsTab(tabName, element) {
    ...
    document.querySelectorAll('.nav-tab').forEach(tab => {
        if (tab.textContent.includes('System Settings') || tab.textContent.includes('AI Configuration')) {
            tab.classList.remove('active');
        }
    });
    
    // Remove active class from all settings content
    ...
    if (tabName === 'system') {
        document.getElementById('systemSettings').classList.add('active');
    } else if (tabName === 'ai') {
        document.getElementById('aiSettings').classList.add('active');
        // Load AI configuration when tab is shown
        if (currentUser && currentUser.role === 'admin') {
            loadAiConfiguration();
        }
    }
...
// ====================
// CSS INJECTION FOR AI SETTINGS (add this to the end of the file)
// ====================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!document.querySelector('#ai-settings-styles')) {
